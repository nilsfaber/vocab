...
VARIANT    = "2_gemma4b_sdxl_refiner"
BASE_DIR   = Path(__file__).resolve().parent              # imagegen/generators/
IMAGEN_DIR = str(BASE_DIR.parent / "words")               # imagegen/words/
...
def build_prompts(entry, scene, keywords=None):
    ...
    # Base positive: scene keywords (with artist appended by Ollama) go into
    # the content CLIP pass; BASE_STYLE goes into a separate ConditioningConcat pass
    base_positive = f"retro-futuristic sci-fi art, {scene_desc}"
...
def phase2_generate_images(data, scenes, vocab, force=False, limit=None):
    ...
    for i, entry in enumerate(pending, 1):
        ...
        base_prompt   = scene_data.get("prompt_base") or scene_data.get("prompt") or build_prompts(entry, scene_data.get("scene", ""), keywords)[0]
        refiner_prompt = scene_data.get("prompt_refiner") or build_prompts(entry, scene_data.get("scene", ""), keywords)[1]
        # Ensure resolved prompts are in scene_data so the manifest entry has them
        ...
        img_path = os.path.join(word_dir, fname)

        with open(img_path, "wb") as f:
            f.write(image_bytes)
        print(f"    saved → {img_path} ({os.path.getsize(img_path)//1024}KB)")
